...
namespace CarvedRock.Backend
{
    ...
    {
        public static List<CyclingFootwear> GetFilteredCyclingFootwear(List<Product> allProducts)
        {
            var list = allProducts.Where(p => p.ProductStatus == ProductStatus.InStock && p.SalePrice > 75).OrderBy(p => p.Name).ThenBy(p => p.SalePrice).ToList();

            IEnumerable<CyclingFootwear> cyclingFootwears = list.OfType<CyclingFootwear>();
            return cyclingFootwears.ToList();
        }

        public static void ExportToJson(List<CyclingFootwear> cyclingFootwears)
        {
            string path = Path.Combine(Environment.CurrentDirectory, "cyclingFootwearInStock.json");

            using (StreamWriter sw = new StreamWriter(path))
            {

                JsonSerializer serializer = new JsonSerializer();
                serializer.Serialize(sw, cyclingFootwears);
            }
